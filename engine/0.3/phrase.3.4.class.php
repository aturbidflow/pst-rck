...
@					$rep = $this->Replacement($text,$i);
		...
		private function Exclusions(){
			$this->text = trim($this->text);
                        ...
                        while ($c > 0){
                            $this->text = preg_replace('/ (a|na|eht) ([a-zA-Z]+) (a|na|eht) /',' $2 $3 ',$this->text, -1, $c);
                        }
                        $this->text = strrev($this->text);                        

                        $this->text = preg_replace('/([a-z][^s]+)s is /','$1s are ',$this->text);
			
                        $this->text = preg_replace('/^a ([a-z]+) everything/',' a $1 something',$this->text);
			$this->text = preg_replace('/ (a|an|the)? another (a|an|the)? ?/',' another ',$this->text);		
                        $this->text = preg_replace('/ another ([a-zA-Z]+[^s])s /',' other $1s ',$this->text);
                        $this->text = preg_replace('/ another ([a-zA-Z]+[^s])s$/',' other $1s',$this->text);
                        $this->text = preg_replace('/ another (a|an) /',' another ',$this->text);
                        $this->text = preg_replace('/^Another (a|an) /','Another ',$this->text);
                        $this->text = preg_replace('/lonely (a|an)/','a lonely',$this->text);
                        $this->text = preg_replace('/ (a)? fallen another /',' another fallen ',$this->text);
			
                        $this->text = preg_replace('/ (a|an) (([a-zA-Z]+) )*([a-zA-Z]+[^s])s$/',' $2 $4s',$this->text);			
			
                        $this->text = preg_replace('/ the (we|us) /',' $1 ',$this->text);
			$this->text = preg_replace('/^the (we|us) /','$1 ',$this->text);
			$this->text = preg_replace('/ the (we|us)$/',' $1',$this->text);
